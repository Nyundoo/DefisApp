...
public class JobPDFExporter extends AbstractExporter {
	...
	private void writeTableData(PdfPTable table, List<Job> listJobs) {
		for(Job job : listJobs) {
			table.addCell(String.valueOf(job.getId()));
			table.addCell(job.getCountry());
			table.addCell(String.valueOf(job.getPayment()));
			table.addCell(job.getJobDescription());
			table.addCell(job.getQualification());
			table.addCell(String.valueOf(job.getInterviewDate()));
			table.addCell(String.valueOf(job.getNoVacancy()));
			table.addCell(String.valueOf(job.getDatePosted()));
		}
