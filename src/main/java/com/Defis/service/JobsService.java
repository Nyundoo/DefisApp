package com.Defis.service;

import java.util.List;
import java.util.NoSuchElementException;

import javax.transaction.Transactional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Sort;
import org.springframework.stereotype.Service;

import com.Defis.domain.Jobs;
import com.Defis.exception.JobsNotFoundException;
import com.Defis.repository.JobsRepository;

@Service
@Transactional
public class JobsService {

public static final int JOBSS_PER_PAGE = 15;

	
	@Autowired
	private JobsRepository jobRepo;
	

	
	public List<Jobs> listAll()	{
		return (List<Jobs>) jobRepo.findAll(Sort.by("id").ascending());
		
	}
	
	public Page<Jobs> listByPage(int pageNum, String sortField, String sortDir, String keyword){
		Sort sort = Sort.by(sortField);
		
		sort = sortDir.equals("asc") ? sort.ascending() : sort.descending();
		
		Pageable pageable = PageRequest.of(pageNum - 1, JOBSS_PER_PAGE, sort);
		
		if(keyword != null) {
			return jobRepo.findAll(keyword, pageable);
		}
		
		return jobRepo.findAll(pageable);
	}
	
	

	public Jobs save(Jobs jobs) {		
	
		return jobRepo.save(jobs);
		
	}
	


	public Jobs get(Integer id) throws JobsNotFoundException {
		try {
			return jobRepo.findById(id).get();
		}catch (NoSuchElementException ex) {
			throw new JobsNotFoundException("Could not find any job with ID " + id);
		}
	}
	
	public void delete(Integer id) throws JobsNotFoundException {
		Long countById = jobRepo.countById(id);
		if(countById == null || countById == 0) {
			throw new JobsNotFoundException("Could not find any job with ID " + id);
		}
		
		jobRepo.deleteById(id);
	}
	
}
